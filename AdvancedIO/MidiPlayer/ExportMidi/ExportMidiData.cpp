...
{
	ofstream outFile;
	outFile.open("MidiData.swift");
	outFile << "// " << midiFilePath << "\n";
	
	int maxTracks = tracks.size() < 4 ? tracks.size() : 4;
	outFile << "// Track size: " << maxTracks << "\n";

	int trackCount = 0;
	for (size_t i = 0; i < tracks.size() && trackCount < maxTracks; i++) {
		...
		outFile << "let midiList" << tracks[i].trackId << ": [MidiEvent] = [\n";
